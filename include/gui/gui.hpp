...
/// is made from trait A to trait B if trait B is defined by a function that
/// uses the value of trait A, thus giving a dependency graph of traits. When a
/// value of one trait is modified (by the implementation or by another trait),
/// the values of all child traits are updated using the new value. Functions
/// may refer to other traits by naming the parent directly or using a selector.
