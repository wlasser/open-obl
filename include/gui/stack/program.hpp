...
/// other traits in the menu. The computational model is that there is a
...
/// the function. This is not always the case, for example
/// (DarN `magic_menu.xml`)
///
/// ```xml
/// <zoom>
...
/// ```
///
/// In the innermost sequence the working value is initially an integer, but
/// then becomes a boolean after the `<eq>` operator. This assumes that the type
/// of the 'mouseover' trait is known to be an integer. For an implementation
/// trait like 'mouseover', the type can be deduced immediately from just the
/// trait name.
///
...
/// TraitTypeId initialized as Unimplemented. During parse time, if a trait is
/// an implementation trait, user trait, or a custom trait with a constant
/// value, then its TraitTypeId is deduced and set. Also during parse time, the
/// operator tree for each trait is translated into an instruction set for a
/// stack machine by performing a post-order DFS. During this phase, selectors
/// are resolved to trait names but types are ignored. Since the (C++) type `T`
/// of each user and implementation trait is known, a `Trait<T>` and
/// `TraitFun<T>` can be constructed. (The `TraitFun<T>` simply evaluates the
/// stack program, expecting an output of type `T`.) The types of non-constant
/// custom traits are not known, but can be deduced from the types of their
/// dependencies during the first update, which are guaranteed to be known due
/// to the topological order. They must therefore be added to the dependency
/// graph as a `Trait<std::variant<...>` with a `TraitFun<std::variant<...>>`.
...
template<class T> void postOrderDFS(pugi::xml_node node, T &&visitor) {
