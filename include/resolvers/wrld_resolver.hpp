...
class World {
 ...
 private:
  ...
  struct DistantChunk {
    ...
    DistantChunk(Ogre::SceneNode *node, Ogre::MaterialPtr matPtr) noexcept
        : node(node), matPtr(std::move(matPtr)) {}
  };
  ...
  struct WaterEntry {
    ...
    WaterEntry(Ogre::SceneNode *node, Ogre::InstancedEntity *entity) noexcept
